 ...
 * @returns {object} 估值结果
 */
function calculatePlateValue(plateNumber) {
  ...
  const doublePattern = /(\d)\1{1}(?!\1)/g // 2连号，但不包含在3连号中
  const doubleMatches = []
  let tempPlate = plate
  // 先标记3连号及以上，避免重复计算
  ...
  const detectNumberSequence = (str) => {
    const sequences = []
  ...
  // 如果没有找到特殊因素，添加通用积极评价
  if (factors.length === 1 && factors[0] === '地区特色（独特标识）') {
    factors.push('简洁明了（好记）')
    factors.push('个性独特（与众不同）')
  }
